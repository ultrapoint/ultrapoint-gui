...
# Base environnement
        ...
        cd ultrapoint/utils/build_scripts
        docker build -f android32.Dockerfile -t ultrapoint-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t ultrapoint-gui-android .
        docker create -it --name ultrapoint-gui-android ultrapoint-gui-android bash

# Get the apk

        docker cp ultrapoint-gui-android:/opt/android/ultrapoint-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/ultrapoint-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
