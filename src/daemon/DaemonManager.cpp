...
{
    // prepare command line arguments and pass to ultrapointd
    ...
    qDebug() << "starting ultrapointd " + m_monerod;
    ...
    while(true && !m_app_exit) {
        ...
        if(running(testnet)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM ultrapointd.exe");
#else
                QProcess::execute("pkill ultrapointd");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./ultrapointd status` returns BUSY when syncing.
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to ultrapointd
#ifdef Q_OS_WIN
    m_monerod = QApplication::applicationDirPath() + "/ultrapointd.exe";
#elif defined(Q_OS_UNIX)
    m_monerod = QApplication::applicationDirPath() + "/ultrapointd";
#endif
