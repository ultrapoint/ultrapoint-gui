...
MONERO_DIR=ultrapoint
MONEROD_EXEC=ultrapointd
...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/ultrapoint-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    MONEROD_EXEC=ultrapointd.exe
fi
...
cd build
qmake ../ultrapoint-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy ultrapointd to bin folder
